...
class CaliperParser:
    ...
    def parse(self, blocks: List[Block]) -> DocumentTree:
        ...
        try:
            logger.info(f"[游标卡尺] ===== 开始解析 (共 {len(blocks)} 个 Block) =====")
            ...
            logger.info(
                f"[游标卡尺] 骨架生成完成 ({total_chars} 字符, "
                f"{len(skeleton_chunks)} 个分片)"
            )
            ...
            logger.info(
                f"[游标卡尺] LLM 识别完成: "
                f"标题='{llm_output.doc_title}', "
                f"章节数={len(llm_output.chapters)}"
            )
            ...
            if llm_output.chapters:
                ...
                if preamble_blocks:
                    ...
                    logger.info(f"[游标卡尺] 前置冗余段: {len(preamble_blocks)} 个 Block")
    ...
    def _deduplicate_overlap_anchors(self, chapters: list) -> list:
        ...
        for ch in chapters[1:]:
            is_dup = False
            # Check against recent anchors (look back up to 5)
