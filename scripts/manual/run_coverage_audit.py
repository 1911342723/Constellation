...
def run_coverage_test():
    ...
    for doc_path in docx_files:
        ...
        def traverse_and_assemble(node, doc_blocks, out_blocks):
            nonlocal assembled_text_chars
            # Header
            if node.title != "Test Root":
                out_blocks.append(f"{'#' * node.level} {node.title}")
            ...
            for child in node.children:
                traverse_and_assemble(child, doc_blocks, out_blocks)
                
        for root_node in assembled_nodes:
            traverse_and_assemble(root_node, blocks, final_md_blocks)
