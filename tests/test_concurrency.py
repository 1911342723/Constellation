...
class TestLRUCacheThreadSafety:
    ...
    def test_concurrent_put_and_get(self):
        ...
        assert not errors, "Data races detected:\n" + "\n".join(errors)
...
class TestSingletonThreadSafety:
    ...
    def test_llm_client_singleton_identity(self):
        ...
        llm_mod._clients.clear()  # reset client pool for test isolation
        ...
        # Same config under concurrent access -> exactly one pooled instance.
        assert len(set(id(inst) for inst in instances)) == 1, (
            "Multiple LLMClient instances created for one config under concurrency"
        )
    ...
    def test_async_llm_client_singleton_identity(self):
        ...
        llm_mod._async_clients.clear()  # reset client pool for test isolation
        ...
        # Same config under concurrent access -> exactly one pooled instance.
        assert len(set(id(inst) for inst in instances)) == 1, (
            "Multiple AsyncLLMClient instances created for one config under concurrency"
        )
...
class TestCompressorParallelism:
    ...
    def test_parallel_matches_serial(self):
        ...
        for i, (s, p) in enumerate(zip(serial_result, parallel_result, strict=True)):
...
class TestAsyncPipeline:
    ...
    def test_async_parse_caching(self):
        ...
        cache_key = parser.cache_key_for(blocks)
        tree = DocumentTree(
            nodes=[],
            doc_title="Cached",
            doc_authors="Test",
...
class TestParserConcurrentAccess:
    ...
    def test_concurrent_parses_with_cache(self):
        ...
        parser_for_key = CaliperParser()
        cache_key = parser_for_key.cache_key_for(blocks)
        tree = DocumentTree(
            nodes=[DocumentNode(
...
class TestLLMOutputValidation:
    """Verify _validate_and_filter_anchors drops invalid anchors."""
    ...
    def test_valid_anchors_pass_through(self):
        ...
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=100)
        assert len(filtered.chapters) == 2

    def test_level_out_of_range_dropped(self):
        result = LLMRouterOutput(
            doc_title="Test",
            doc_authors="",
            chapters=[
                ChapterNode(title="Ch1", start_block_id=0, level=1, snippet="Ch1"),
                ChapterNode(title="Bad", start_block_id=5, level=0, snippet="Bad"),
                ChapterNode(title="Bad2", start_block_id=10, level=7, snippet="Bad2"),
            ],
        )
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=100)
        assert len(filtered.chapters) == 1
        assert filtered.chapters[0].title == "Ch1"

    def test_negative_block_id_dropped(self):
        result = LLMRouterOutput(
            doc_title="Test",
            doc_authors="",
            chapters=[
                ChapterNode(title="Bad", start_block_id=-1, level=1, snippet="Bad"),
            ],
        )
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=100)
        assert len(filtered.chapters) == 0

    def test_block_id_exceeds_max_dropped(self):
        result = LLMRouterOutput(
            doc_title="Test",
            doc_authors="",
            chapters=[
                ChapterNode(title="Bad", start_block_id=200, level=1, snippet="Bad"),
            ],
        )
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=100)
        assert len(filtered.chapters) == 0
    ...
    def test_empty_title_dropped(self):
        result = LLMRouterOutput(
            ...
            doc_authors="",
            chapters=[
                ChapterNode(title="  ", start_block_id=0, level=1, snippet=""),
                ChapterNode(title="Good", start_block_id=5, level=1, snippet="G"),
            ],
        )
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=100)
        assert len(filtered.chapters) == 1
        assert filtered.chapters[0].title == "Good"

    def test_no_max_block_id_skips_range_check(self):
        """When max_block_id=-1, block_id range check is skipped."""
        result = LLMRouterOutput(
            doc_title="Test",
            doc_authors="",
            chapters=[
                ChapterNode(title="Ch1", start_block_id=9999, level=1, snippet="Ch1"),
            ],
        )
        filtered = LLMRouter._validate_and_filter_anchors(result, max_block_id=-1)
        assert len(filtered.chapters) == 1


# ═══════════════════════════════════════════════════════════════
# 12. Multi-window hierarchy normalization
# ═══════════════════════════════════════════════════════════════

class TestMultiWindowHierarchy:
    """Verify _normalize_hierarchy fixes cross-window level jumps."""

    def _make_blocks_with_fonts(self) -> list:
        """Create blocks with distinct font sizes for L1 (16pt) and L2 (12pt)."""
        blocks = []
        for i in range(100):
            blocks.append(Block(
                id=i, type="text", text=f"Block {i}",
                font_size=16.0 if i % 20 == 0 else 12.0,
                is_bold=(i % 20 == 0),
            ))
        return blocks

    def test_level_jump_fixed(self):
        """A chapter jumping from L1 to L3 should be resolved (clamped or font-assisted)."""
        parser = CaliperParser()
        blocks = self._make_blocks_with_fonts()

        chapters = [
            ChapterNode(title="Ch1", start_block_id=0, level=1, snippet="Ch1"),
            ChapterNode(title="Ch2", start_block_id=20, level=3, snippet="Ch2"),
        ]

        fixed = parser._normalize_hierarchy(chapters, blocks)
        # Should be resolved to <= 2 (either clamped to 2 or font-assisted to 1)
        assert fixed[1].level <= 2

    def test_sibling_promotion(self):
        """Consecutive same-font-size headings should be siblings, not nested."""
        parser = CaliperParser()
        blocks = self._make_blocks_with_fonts()

        chapters = [
            ChapterNode(title="Ch1", start_block_id=0, level=1, snippet="Ch1"),
            # L2 with 16pt font — same as L1, should be promoted to L1
            ChapterNode(title="Ch2", start_block_id=20, level=2, snippet="Ch2"),
        ]
        # Give block 20 the same font size as block 0
        blocks[20].font_size = 16.0

        fixed = parser._normalize_hierarchy(chapters, blocks)
        assert fixed[1].level == 1

    def test_single_chapter_unchanged(self):
        """Single chapter should pass through unchanged."""
        parser = CaliperParser()
        blocks = self._make_blocks_with_fonts()
