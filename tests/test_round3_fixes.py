...
class TestCharRecallRecursive:
    ...
    def test_image_base64_excluded_from_char_count(self):
        prose = "Real body text."
        big_b64 = "A" * 100_000
        node = DocumentNode(
            title="Figures", level=1, start_block_id=0, end_block_id=2,
            content=f"{prose}\n\n![Fig 1](data:image/png;base64,{big_b64})",
            children=[],
        )
        tree = DocumentTree(nodes=[node])
        stats = tree.get_stats()
        # Only prose counts; the base64 payload (and image markup) is excluded.
        assert stats["total_content_chars"] == len(f"{prose}\n\n")
...
class TestCacheKeyPhysicalFeatures:
    ...
    def test_different_compressor_config_different_hash(self):
        blocks = [Block(id=0, type="text", text="Hello")]
        h1 = _compute_blocks_hash(blocks, compressor_config=CompressorConfig(head_chars=40))
        h2 = _compute_blocks_hash(blocks, compressor_config=CompressorConfig(head_chars=80))
        assert h1 != h2

    def test_different_resolver_config_different_hash(self):
        blocks = [Block(id=0, type="text", text="Hello")]
        h1 = _compute_blocks_hash(blocks, resolver_config=ResolverConfig(fuzzy_anchor_radius=5))
        h2 = _compute_blocks_hash(blocks, resolver_config=ResolverConfig(fuzzy_anchor_radius=15))
        assert h1 != h2

    def test_different_llm_model_different_hash(self):
        blocks = [Block(id=0, type="text", text="Hello")]
        h1 = _compute_blocks_hash(blocks, llm_model="deepseek-chat")
        h2 = _compute_blocks_hash(blocks, llm_model="gpt-4o")
        assert h1 != h2

    def test_no_config_backward_compatible(self):
        """Calling without configs should still work (backward compat)."""
        blocks = [Block(id=0, type="text", text="Hello")]
        h1 = _compute_blocks_hash(blocks)
        h2 = _compute_blocks_hash(blocks)
        assert h1 == h2
...
class TestLLMNullSafety:
    ...
    def test_build_messages_uses_user_role_for_gateway_compatibility(self):
        ...
        messages = LLMClient._build_messages(
            ...
            LLMRouterOutput,
            "Return a chapter hierarchy.",
        )

        assert [message["role"] for message in messages] == ["user"]
        assert "Return a chapter hierarchy." in messages[0]["content"]
        assert "Expected JSON Schema:" in messages[0]["content"]
        assert "Extract chapters from blocks." in messages[0]["content"]
...
class TestResolverDeepCopy:
    ...
    def test_multiple_resolves_produce_consistent_results(self):
        ...
        for n1, n2 in zip(result1, result2, strict=True):
